...
SSL_RENEWAL_DAYS=30
SSL_MIN_CERT_DAYS=$((SSL_RENEWAL_DAYS + 2))
SSL_MIN_CA_CERT_DAYS=$((SSL_MIN_CERT_DAYS + 1))
CA_REUSE_MIN_SECONDS=$((SSL_MIN_CERT_DAYS * 86400))
...
if [ "$SSL_CERT_DAYS" -lt "$SSL_MIN_CERT_DAYS" ]; then
  echo "WARNING: SSL_CERT_DAYS must be at least $SSL_MIN_CERT_DAYS days; using $DEFAULT_SSL_CERT_DAYS." >&2
  SSL_CERT_DAYS=$DEFAULT_SSL_CERT_DAYS
fi

if [ "$SSL_CA_CERT_DAYS" -lt "$SSL_MIN_CA_CERT_DAYS" ]; then
  echo "WARNING: SSL_CA_CERT_DAYS must be at least $SSL_MIN_CA_CERT_DAYS days; using $DEFAULT_SSL_CA_CERT_DAYS." >&2
